...
(function(){
  ...
  function calc(){
    const rhoI=gv('rhoI')/100,rhoS=gv('rhoS')/100,n=gv('n'),G=gv('G'),H0=gv('H0');
    document.getElementById('sim-rhoIv').textContent=(rhoI).toFixed(2);
    document.getElementById('sim-rhoSv').textContent=(rhoS).toFixed(2);
    ['n','G','H0'].forEach(k=>document.getElementById('sim-'+k+'v').textContent=gv(k));
    ...
    document.getElementById('sim-eq').textContent=`H(t+1) = H(t)·(1−${rhoI.toFixed(2)}−${n}×${rhoS.toFixed(2)}) = H(t)·${(1-rhoTotal).toFixed(3)}`;
